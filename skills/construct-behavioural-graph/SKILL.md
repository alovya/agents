...
**Leaf nodes in the JSON need not be the genuinely-lowest-level behaviours.** The JSON uses leaf nodes merely as programmatic building blocks to build the graph: they are not necessarily the actual lowest-level behaviours. For example, a composite node contains two or more nodes (leaf or composite), but a leaf node may still contain finer behaviour conceptually. In other words, the leaf nodes you generate need not be the semantically or code-syntactically lowest-level behaviours or implementation details and in fact this is **strongly disrecommended**: **never** go down to the lowest level at first, go only as low as the user asks, only go lower if requested, and only go lower on the behaviour(s) the user is talking about - leave other behaviours at a higher level.