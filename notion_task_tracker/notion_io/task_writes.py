...
from notion_task_tracker.notion_io.markdown import bullet, heading, join_markdown_blocks, page_mention, toggle
...
from notion_task_tracker.tasks.pages.landing_pages import (
...
def page_registry_for_task_graph(task_graph: TaskDependencyGraph) -> NotionPageRegistry:
    return NotionPageRegistry.from_tracked_pages(_pages_that_should_exist(task_graph))


def notion_write_plan_for_task_graph(task_graph: TaskDependencyGraph) -> list[NotionWriteIntent]:
    task_graph.validate()
    task_graph.recalculate_display_priorities()
    page_registry = page_registry_for_task_graph(task_graph)
    return [
        *_missing_page_creation_intents(task_graph, page_registry),
        *_fixed_page_title_refresh_intents(task_graph),
        *_existing_task_property_refresh_intents(task_graph),
        ongoing_landing_page_refresh_intent(task_graph, page_registry),
        *completed_landing_page_refresh_intents(task_graph, page_registry),
    ]


def timeline_log_write_intent(timeline_log_change: TimelineLogChange) -> NotionWriteIntent:
    ...
    arguments = {
        ...
        "timeline_section_markdown": timeline_entry_to_timeline_section_markdown(timeline_entry),
    }
    if timeline_log_change.existing_timeline_entry is not None:
        ...
        arguments["old_timeline_section_markdown"] = timeline_entry_to_timeline_section_markdown(
            timeline_log_change.existing_timeline_entry
        )
        arguments["new_timeline_section_markdown"] = timeline_entry_to_timeline_section_markdown(timeline_entry)
        arguments["appended_markdown"] = timeline_entry_to_content_markdown(
...
def completion_write_intents(
    task_graph: TaskDependencyGraph,
    completion_change: TaskCompletionChange,
) -> list[NotionWriteIntent]:
    page_registry = page_registry_for_task_graph(task_graph)
    return [
        task_database_property_refresh_intent(task_graph.tasks[completion_change.task_id]),
        ongoing_landing_page_refresh_intent(task_graph, page_registry),
        *completed_landing_page_refresh_intents(task_graph, page_registry),
        timeline_log_write_intent(completion_change.timeline_log_change),
    ]


def task_database_property_refresh_intent(task: Task) -> NotionWriteIntent:
    return NotionWriteIntent(
...
def ongoing_landing_page_refresh_intent(
    task_graph: TaskDependencyGraph,
...
def completed_landing_page_refresh_intents(
    task_graph: TaskDependencyGraph,
    page_registry: NotionPageRegistry,
) -> list[NotionWriteIntent]:
...
def render_ongoing_landing_page_markdown(
    ...
    markdown_blocks = []
    for priority, task_ids in _task_ids_grouped_by_priority(tasks).items():
        if task_ids:
...
def timeline_entry_to_content_markdown(timeline_entry: TimelineEntry) -> str:
...
def timeline_entry_to_timeline_section_markdown(timeline_entry: TimelineEntry) -> str:
    return join_markdown_blocks([
        heading(3, timeline_entry.heading),
        timeline_entry_to_content_markdown(timeline_entry),
    ])


def _missing_page_creation_intents(
    task_graph: TaskDependencyGraph,
    page_registry: NotionPageRegistry,
) -> list[NotionWriteIntent]:
    write_intents = []
    for page, markdown in [
        ...
        if page.notion_page_id is None:
            write_intents.append(_page_creation_intent(page, markdown))
    return write_intents


def _fixed_page_title_refresh_intents(task_graph: TaskDependencyGraph) -> list[NotionWriteIntent]:
    return [
        _page_title_refresh_intent(page)
        for page in [
            task_graph.ongoing_tasks_landing_page.page,
            task_graph.completed_tasks_landing_page.page,
        ]
...
def _existing_task_property_refresh_intents(task_graph: TaskDependencyGraph) -> list[NotionWriteIntent]:
    return [
        task_database_property_refresh_intent(task)
...
def _page_creation_intent(page: TrackedPage, markdown: str) -> NotionWriteIntent:
    return NotionWriteIntent(
...
def _page_title_refresh_intent(page: TrackedPage) -> NotionWriteIntent:
    return NotionWriteIntent(
...
def _pages_that_should_exist(task_graph: TaskDependencyGraph) -> list[TrackedPage]:
    pages = [
        task_graph.ongoing_tasks_landing_page.page,
        task_graph.completed_tasks_landing_page.page,
    ]
...
def _task_ids_grouped_by_priority(tasks: dict[str, Task]) -> dict[Priority, list[str]]:
    return {
        priority: [
            task_id
